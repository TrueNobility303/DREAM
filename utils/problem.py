...
# GT-DA
def DA(data, label, x0, y0, args, comm, epochs):
    alpha = args.alpha
    lambda2 = args.lambda2
    kappa = args.kappa
    ...
    for epoch in tqdm(range(epochs)):
        ...
        gy_old = np.copy(gy)
        # update x and y
        if epoch % args.multi_step == 0:
            x_old = np.copy(x)
            x = x - args.lr_x * vx
        else:
            y_old = np.copy(y)
            y = y + args.lr_y * vy
            # we ignore the gradient tracking step in y direction following option (2) in the paper.
        ...
        if (epoch + 1) % args.print_freq == 0:
            ...
            if rank == 0:
                ...
                phi,grad_phi = cal_phi(data, label, x_bar, n, lambda2, alpha)
                with open(args.out_fname, '+a') as f:
                    print('GT-DA:{ep:d},{phi:.8f},{grad_phi:.8f}'.format(ep=epoch + 1, phi=float(phi),grad_phi=float(grad_phi)), file=f)
                # store the minimal loss
                if phi < min_phi:
                    min_phi = phi
                phi_lst.append(phi / n)
...
def DMHSGD(data, label, x0, y0, args, comm, epochs):
    alpha = args.alpha
    lambda2 = args.lambda2
    kappa = args.kappa
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if (epoch + 1) % args.print_freq == 0:
            ...
            if rank == 0:
                ...
                phi,grad_phi = cal_phi(data, label, x_bar, n, lambda2, alpha)
                with open(args.out_fname, '+a') as f:
                    print('DMHSGD: {ep:d},{phi:.8f},{grad_phi:.8f}'.format(ep=epoch + 1, phi=float(phi),grad_phi=float(grad_phi)), file=f)
                # store the minimal loss
                if phi < min_phi:
                    min_phi = phi
                phi_lst.append(phi / n)
...
def DREAM(data, label, x0, y0, args, comm, epochs):
    alpha = args.alpha
    lambda2 = args.lambda2
    kappa = args.kappa
    ...
    for epoch in tqdm(range(epochs)):
        # decentralized minimax optimizer 
        # args b0,b, prob and q
        
        rad1 = np.random.random()
        
        if rad1 < args.prob:
            id0 = np.random.randint(0, p, args.b0) * rank
            # id0 = range(n)
            gx = cal_gradient_x(data, label, x, y, id0, lambda2, alpha)
            gy = cal_gradient_y(data, label, x, y, id0, n)
            grad_cnt = grad_cnt + 2 * size * args.b0
        else:
            rad2 = np.random.random()
            if rad2 < args.q:
                idx = np.random.randint(0, p, args.b) * rank
                gx = cal_gradient_x(data, label, x, y, idx, lambda2, alpha) + \
                    1/args.q * (gx_old - cal_gradient_x(data, label, x_old, y_old, idx, lambda2, alpha))
                gy = cal_gradient_y(data, label, x, y, idx, n) + \
                    1/args.q * (gy_old - cal_gradient_y(data, label, x_old, y_old, idx, n)) 
                grad_cnt = grad_cnt + 4 * size * args.b
            else:
                gx = gx_old 
                gy = gy_old
        ...
        if (epoch + 1) % args.print_freq == 0:
            ...
            if rank == 0:
                ...
                phi,grad_phi = cal_phi(data, label, x_bar, n, lambda2, alpha)
                with open(args.out_fname, '+a') as f:
                    print('DREAM: {ep:d},{phi:.8f},{grad_phi:.8f}'.format(ep=epoch + 1, phi=float(phi),grad_phi=float(grad_phi)), file=f)
                # store the minimal loss
                if phi < min_phi:
                    min_phi = phi
                phi_lst.append(phi / n)
